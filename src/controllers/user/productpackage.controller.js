...
exports.getProductinPackage = async (req, res) => {
  try {
    const productInpackageid = await ProductInPackage.findAll({
      where: {
        proudcutInPackagedId
      },
    })
    const findproductPackageId = await ProductPackage.findOne({
      where: {
          productPackagedId: productInpackageid[0].productPackagedId
      }
    })
    const productId = await Products.findOne({
      where: {
        productPackageId: findproductPackageId.productId
      }
    })
    return res.render('./productpackageid', {findproductPackageId, productId})
  }catch (error){
    console.error('Function getProductPackageId Error: ', error);
    return res.render('404');
  }
};
